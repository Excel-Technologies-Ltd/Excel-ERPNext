...
def _execute(filters, additional_table_columns=None, additional_query_columns=None):
	...
	for inv in invoice_list:
		...
		row.update({
			...
			'excel_invoice_type': inv.excel_invoice_type,
...
def get_columns(invoice_list, additional_table_columns):
	...
	columns +=[
		...
		{
			"fieldname": "excel_invoice_type",
			"label": _("Excel Invoice Type"),
			"fieldtype": "Data",
			"width": 120
		},
		{
...
def get_conditions(filters):
	...
	if filters.get("excel_invoice_type"): conditions += " and excel_invoice_type = %(excel_invoice_type)s"
	...
	if accounting_dimensions:
		common_condition = """
			and exists(select name, from `tabSales Invoice Item`, `tabSales Team`
				where parent=`tabSales Invoice`.name 
			"""
...
def get_invoices(filters, additional_query_columns):
	...
	all_data_from_sql= frappe.db.sql("""
		select tsi.name, tsi.posting_date, tsi.debit_to, tsi.project, tsi.customer, tst.sales_person,
		tsi.customer_name, tsi.owner, tsi.remarks, tsi.territory, tsi.tax_id, tsi.customer_group,
		tsi.base_net_total, tsi.base_grand_total, tsi.base_rounded_total, tsi.excel_invoice_type, tsi.outstanding_amount {0}
		from `tabSales Invoice` as tsi left join `tabSales Team` as tst on tsi.name = tst.parent
		where tsi.docstatus = 1 %s order by tsi.posting_date desc, tsi.name desc""".format(additional_query_columns or '') %
		conditions, filters, as_dict=1)

	#print("\n\n",all_data_from_sql,"\n\n")

	return all_data_from_sql
